...
// when we receive a sigint go on to another prompt, and tell the user how to
// quit.
void interrupt_handler(int signum) {
    printf(" %s\n%% ", "to quit myshell use 'exit'");
    fflush(stdout);
}
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        if ( argc <= 1 ) printf("%% "); /* printf uses %d, %s, %x, etc.  See 'man 3 printf' */
