...
for ball in balls:
	...
	#creates the ball
        ...
        while i < 10:
            ...
            shaderSet = cmds.sets(renderable=True, noSurfaceShader=True, empty=True, name= myShader + 'SG')
            cmds.setAttr( myShader +'.color',0,1 ,0, type='double3')
            cmds.setAttr( myShader +'.incandescence', 0, 0, 1, type = 'double3')
            cmds.connectAttr(myShader +".outColor", shaderSet+".surfaceShader")
            cmds.sets(forceElement=shaderSet, e=True)
...
for ball in balls:
	...
	#creates the ball
        n=cmds.sphere( name = 'ball', p=(x,y,z), radius = num )
...
# Press the ESC key to stop playback. # 
select -cl  ;
select -r front side a b c d e g ;
select -cl  ;
select -r a b d g h i j k l s t v w x c1 j1 l1 n1 o1 q1 t1 v1 w1 x1 y1 z1 ;
select -cl  ;
select -r a d g h l s t x y c1 h1 j1 k1 l1 n1 o1 q1 w1 y1 z1 ;
select -cl  ;
select -r a d g h l s t x y c1 h1 j1 k1 l1 n1 o1 q1 w1 y1 z1 ;
cmds.select(balls[0:10])
a = cmds.select(balls[0:10])
print a
None
select -cl  ;
a = cmds.select(balls[0:10])
...
# Error: ValueError: No object matches name: ['a', 'b', 'c', 'd', 'e', 'g', 'h', 'i', 'j', 'k'] # 
select -cl  ;
import maya.cmds as cmds
import random
...
myShader = cmds.shadingNode('blinn', n='blue_balls', asShader=True)
myShader1 = cmds.shadingNode('blinn', n='pink_balls', asShader=True)
...
cmds.select(balls[32:])
shaderSet2 = cmds.sets(renderable=True, noSurfaceShader=True, empty=True, name= myShader + 'SG')
cmds.setAttr( myShader2 +'.color', 0.0, 0 , 0.5, type='double3')
cmds.setAttr( myShader2 +'.incandescence', 0.0, 0, 0.5 , type = 'double3')
cmds.connectAttr(myShader2 +".outColor", shaderSet2+".surfaceShader")
cmds.sets(forceElement=shaderSet2, e=True)
...
cmds.select(balls[31:])
shaderSet2 = cmds.sets(renderable=True, noSurfaceShader=True, empty=True, name= myShader + 'SG')
cmds.setAttr( myShader2 +'.color', 0.0, 0 , 1, type='double3')
cmds.setAttr( myShader2 +'.incandescence', 0.0, 0, 0.5 , type = 'double3')
cmds.connectAttr(myShader2 +".outColor", shaderSet2+".surfaceShader")
cmds.sets(forceElement=shaderSet2, e=True)
...
cmds.select(balls[31:])
shaderSet2 = cmds.sets(renderable=True, noSurfaceShader=True, empty=True, name= myShader + 'SG')
cmds.setAttr( myShader2 +'.color', 0.0, 0.5 , 0, type='double3')
cmds.setAttr( myShader2 +'.incandescence', 0.0, .5, 0.0 , type = 'double3')
cmds.connectAttr(myShader2 +".outColor", shaderSet2+".surfaceShader")
cmds.sets(forceElement=shaderSet2, e=True)
