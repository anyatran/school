...
This script is converted from a mel script that builds a flower. This will have customized shaders to color the object and translate across the x-axis.
'''
...
''' Build path '''
path = []
path.append(range(0,10))
path.append(range(10, -10, -1))
path.append(range(-10, 1, 1))
...
for block in path:
	for vertex in block:
		mc.setKeyframe("Flower", at="translateX", v=vertex, t=frameCount)
...
mc.playbackOptions(maxTime=4000)
mc.play(forward=1)
