...
(defunc f (l)
  ...
  :output-contract (natlistp (f l))
  (cond ((endp l) nil)
        ((equal 1 (len l)) nil)
        (t 
         (if (evenp (first (rest l))) (cons (first l) (f (rest l)))
           (f (rest l))))))

(check= (f '(3)) nil)
(check= (f '(3 6)) '(3))
(check= (f '(3 6 2 1 0 8 9)) '(3 6 1 0))

(defunc ft (l acc)
  :input-contract (and (natlistp l) (natlistp acc))
  :output-contract (natlistp (ft l acc))
  (cond ((endp l) acc)
        ((equal 1 (len l)) acc)
        (t 
         (if (evenp (first (rest l))) (ft (rest l) (cons (first l) acc))
           (ft (rest l) acc)))))

(check= (ft '(3) nil) nil)
(check= (ft '(3 6) nil) '(3))
(check= (ft '(9 0 2 1 3 3 4) '(4 5 6)) '(3 0 9 4 5 6))#|ACL2s-ToDo-Line|#
