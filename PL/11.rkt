...
;; max-meat : Nat Nat -> Nat
;; returns zero if the first number is larger,
;; one if the second number is larger
(define/rec max-meat
  (lambda (x) 
    (lambda (y)
      (if (zero? x)
...
;; ref : Nat (Listof A) -> A
;; returns the nth item of the given list 
;;(note: this doesn't have to be
;; a recursive function, but in a way that doesn't 
;;correspond to plain
;; racket)
(define/rec ref
  (lambda (n)
...
(define/rec inter
  (lambda (l)
    (lambda (x)
      (lambda (r)
        (if (null? r)
            (cons (append l (append x r)) null)
            (cons (append l (append x r))
                  (inter (append l (cons (car r) null)) x (cdr r))))))))
...
(define/rec permutations
  (lambda (l)
    (if (null? l)
        ...
        ;; `permutations'
        (append* (map (lambda (x) (interleave (car l) x))
                      (permutations (cdr l)))))))
;; tests
...
(define/rec unique?
  (lambda (l)
    (or (null? l)
        (and (not (member? (car l) (cdr l))) (unique? (cdr l))))))
;; tests
...
(define von-koch
  (lambda (edges)
    (with [n (add1 (length edges))]
      ...
      (filter (lambda (x) (graceful? edges x)) assignments)))))
